            ...
            foreach (var row in lines)
            {
                ...
                for (var i = 0; i < headers.Length; i++)
                {
                    foreach (var col in cols)
                    {
                        ...
                        {
                            // Normalize.RemoveSpecialCharacters(values[1]
                            object value;
                            if (col.Name == "RegionPopulation")
                            {
                                value = Convert.ChangeType(Normalize.RemoveSpecialCharacters(vals[i]), col.PropertyType);
                            }
                            else
                            {
                                value = Convert.ChangeType(vals[i], col.PropertyType);
                            }
                            col.SetValue(entry, value);
                        }
                    }
                }
